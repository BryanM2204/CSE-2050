...
class Game():
    ...
    def find_route(self, currow, curcol, curscore, curpath):
        '''Recursively find the route through the maze. Return the score and path'''
        #Base case - if the current row and column are the finish, return the score and path
        if self._is_puzzle_solved(currow, curcol): return curscore, curpath

        #If the current path is empty, add the starting position to it
        if len(curpath) == 0: curpath.append((currow, curcol))
        
        #Get the possible moves
        moves = [(currow, curcol + 1), (currow, curcol - 1), (currow + 1, curcol), (currow - 1, curcol)]

        #Initialize the max score and path
        max_score = -1
        max_path = []

        #Loop through the possible moves
        for move in moves:
            #If the move is available, not a wall, and in the maze, add it to the path and update the score
            if self._is_move_available(move[0], move[1], curpath) and self._maze.is_move_in_maze(move[0], move[1]) and not self._maze.is_wall(move[0], move[1]):
                #Add the value of the current square to the score
                newscore = curscore + self._maze._maze[move[0]][move[1]]
                #Creates a copy of the current path and adds the current move to it
                newpath = curpath.copy()
                newpath.append(move)

                #Recursively call the function with the new score and path
                score, path = self.find_route(move[0], move[1], newscore, newpath)

                #If the score is greater than the max score, update the max score and path
                if score > max_score:
                    max_score = score
                    max_path = path

        #Return the max score and path
